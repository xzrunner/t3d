...
// 	sound.SetVolume(wind_sound_id, 100);
	...
	BmpFile* bitmap = new BmpFile("../../assets/chap14/lego02.BMP");
	cockpit->LoadFrame(bitmap,0,0,0,BITMAP_EXTRACT_MODE_ABS);
	delete bitmap;

	bitmap = new BmpFile("../../assets/chap14/lego02b.BMP");
	cockpit->LoadFrame(bitmap,1,0,0,BITMAP_EXTRACT_MODE_ABS);
	delete bitmap;
...
//	lights[POINT_LIGHT2_INDEX].pos.y = 400;
	...
	else
	{
		...
		// compute vertex indices into vertex _list of the current quad
		v0 = cell_x + cell_y*obj_terrain->GetIVar2();
		v1 = v0 + 1;
		v2 = v1 + obj_terrain->GetIVar2();
		v3 = v0 + obj_terrain->GetIVar2();
		...
		// update position
		shadow_obj->SetWorldPos(shadow_obj->GetWorldPos().x,
							    terrain_height+25,
								shadow_obj->GetWorldPos().z);
		...
		shadow_obj->SetWorldPos(pl.x + t*vlo.x,
			                    shadow_obj->GetWorldPos().y,
								pl.z + t*vlo.z);
		...
		rs = ks * ( obj_work->GetAvgRadius() * (hl/ (hl - obj_work->GetWorldPos().y)) );
		...
		vec4 vd = shadow_obj->GetWorldPos() - _cam->Pos();
		float d = vd.LengthFast();

		// test if distance is within 1.5 times the radius of the shadow, the 1.5 makes it
		// a little more noticeable
		if (d < 1.5*rs)
			cockpit->SetCurrFrame(1); // cockpit with lower brightness
		...
		hl = lights[POINT_LIGHT2_INDEX].pos.y;
		...
		// test if camera is in shadow
		vd = shadow_obj->GetWorldPos() - _cam->Pos();
		d = vd.LengthFast();
