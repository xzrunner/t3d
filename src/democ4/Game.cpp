...
#define INTRO_STATE_COUNT1 (30*3)
#define INTRO_STATE_COUNT2 (30*5)
#define INTRO_STATE_COUNT3 (30*7)
...
// 	Build_Sin_Cos_Tables();
	...
	obj_terrain->GenerateTerrain(TERRAIN_WIDTH,				// width in world coords on x-axis
								...
								"../../assets/chap12/water_track_height_04.bmp",  // filename of height bitmap encoded in 256 colors
								"../../assets/chap12/water_track_color_03.bmp",   // filename of texture map
	...
	obj_terrain2->GenerateTerrain(TERRAIN_WIDTH,				// width in world coords on x-axis
								 TERRAIN_HEIGHT,			// height (length) in world coords on z-axis
								 TERRAIN_SCALE,				// vertical scale of terrain
								"../../assets/chap12/heightmap02.bmp",  // filename of height bitmap encoded in 256 colors
								"../../assets/chap12/seabottom_01.bmp",   // filename of texture map
								_RGB32BIT(255,255,255,255), // color of terrain if no texture        
								&terrain_pos,				// initial position
								NULL,						// initial rotations
								POLY_ATTR_RGB24 | 
	...
	obj_player->LoadCOB("../../assets/chap12/jetski06.cob", 
...
// 		rgblookup);          // lookup table
	...
	wind_sound_id           = sound.LoadWAV("../../assets/chap12/WIND.WAV");
	water_light_sound_id    = sound.LoadWAV("../../assets/chap12/WATERLIGHT01.WAV");
	water_heavy_sound_id    = sound.LoadWAV("../../assets/chap12/WATERHEAVY01.WAV");
	water_splash_sound_id   = sound.LoadWAV("../../assets/chap12/SPLASH01.WAV");
	zbuffer_intro_sound_id  = sound.LoadWAV("../../assets/chap12/ZBUFFER02.WAV");
	zbuffer_instr_sound_id  = sound.LoadWAV("../../assets/chap12/ZINSTRUCTIONS02.WAV");
	jetski_start_sound_id   = sound.LoadWAV("../../assets/chap12/jetskistart04.WAV");
	jetski_idle_sound_id    = sound.LoadWAV("../../assets/chap12/jetskiidle01.WAV");
	jetski_fast_sound_id    = sound.LoadWAV("../../assets/chap12/jetskifast01.WAV");
	jetski_accel_sound_id   = sound.LoadWAV("../../assets/chap12/jetskiaccel01.WAV");
	jetski_splash_sound_id  = sound.LoadWAV("../../assets/chap12/splash01.WAV");
	...
	// intro
	bitmap = new BmpFile("../../assets/chap12/zbufferwr_intro01.BMP");
	...
	// get ready
	bitmap = new BmpFile("../../assets/chap12/zbufferwr_ready01.BMP");
	...
	// nice one
	bitmap = new BmpFile("../../assets/chap12/zbufferwr_nice01.BMP");
...
void Game::Step()
{
	...
	static bool seafloor_mode  = true;
...
// 				    sound.SetVolume(jetski_fast_sound_id, 0);
			...
			vec4 pos = obj_terrain2->GetWorldPos();
			pos.y = -220;
			obj_terrain2->SetWorldPos(pos.x, pos.y, pos.z);
			obj_terrain2->ModelToWorld(TRANSFORM_LOCAL_TO_TRANS);
			...
			lights[POINT_LIGHT_INDEX].pos.x = 170*cos(DEG_TO_RAD(_cam->Dir().y));
			lights[POINT_LIGHT_INDEX].pos.y = 50;
			lights[POINT_LIGHT_INDEX].pos.z = 170*sin(DEG_TO_RAD(_cam->Dir().y));
			...
			obj_player->SetWorldPos(_cam->Pos().x + (130+cam_speed*1.75)*sin(DEG_TO_RAD(_cam->Dir().y)),
									_cam->Pos().y - 20 + 7.5*sin(wave_count),
...
// 				sound.SetVolume(jetski_splash_sound_id, fabs(cam_speed)*5);
			...
			// insert terrain objects into render list, check if sea floor is disabled
 			if (seafloor_mode == 1)
 				_list->Insert(*obj_terrain2,false);
...
// 				_list->DrawSolidZB32(graphics.GetBackBuffer(), graphics.GetBackLinePitch(), zbuffer->Buffer(), WINDOW_WIDTH*4);
			...
			else
			{
				...
				else
				{
					...
					// set up rendering context
					RenderContext rc;
				   rc.attr         = /*RENDER_ATTR_ZBUFFER*/RENDER_ATTR_INVZBUFFER  
									 | RENDER_ATTR_ALPHA  
									 //| RENDER_ATTR_MIPMAP  
									 | RENDER_ATTR_TEXTURE_PERSPECTIVE_AFFINE;
