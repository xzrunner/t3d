...
// 	Build_Sin_Cos_Tables();
	...
	obj_terrain->GenerateTerrain(TERRAIN_WIDTH,				// width in world coords on x-axis
								...
								"../../assets/chap14/grass256_256_01.bmp",   // filename of texture map
...
// 	sound.SetVolume(wind_sound_id, 100);
	...
	BmpFile* bitmap = new BmpFile("../../assets/chap14/lego01.BMP");
...
//	lights[POINT_LIGHT2_INDEX].pos.y = 400;
	...
	obj_work->SetIVar1(obj_work->GetIVar1() + 3);

	if (obj_work->GetIVar1() >= 360)
		obj_work->SetIVar1(0);

	// set position of object 
	obj_work->SetWorldPos(250*cos(DEG_TO_RAD(obj_work->GetIVar1())),
						  200+125*sin(DEG_TO_RAD(3*obj_work->GetIVar1())),
						  250*sin(DEG_TO_RAD(obj_work->GetIVar1())));
	...
	// reset the object (this only matters for backface and object removal)
	obj_light_array[INDEX_WHITE_LIGHT_INDEX]->Reset();

	// set position of object to light
	obj_light_array[INDEX_WHITE_LIGHT_INDEX]->SetWorldPos(
	...
	// rotate the local coords of the object
	obj_light_array[INDEX_WHITE_LIGHT_INDEX]->Transform(mrot, TRANSFORM_LOCAL_TO_TRANS,1);

	// perform world transform
	obj_light_array[INDEX_WHITE_LIGHT_INDEX]->ModelToWorld(TRANSFORM_TRANS_ONLY);

	// insert the object into render _list
	_list->Insert(*obj_light_array[INDEX_WHITE_LIGHT_INDEX],0);
	...
	else
	{
		...
		cell_x = (obj_work->GetWorldPos().x  + TERRAIN_WIDTH/2) / obj_terrain->GetFVar1();
		cell_y = (obj_work->GetWorldPos().z  + TERRAIN_HEIGHT/2) / obj_terrain->GetFVar1();
		...
		// now simply index into table 
		terrain_height = max(max(obj_terrain->GetTransVertex(v0).y, obj_terrain->GetTransVertex(v1).y), 
			max(obj_terrain->GetTransVertex(v2).y, obj_terrain->GetTransVertex(v3).y) );

		// update position
		shadow_obj->SetWorldPos(obj_work->GetWorldPos().x,
							    terrain_height+25,
								obj_work->GetWorldPos().z);
		...
		hl = lights[POINT_LIGHT_INDEX].pos.y;

		// use the average radius of the object to base calcs on, better than nothing
		float rs = ks * ( obj_work->GetAvgRadius() * (hl/ (hl - obj_work->GetWorldPos().y)) );
		...
		shadow_obj->Transform(mrot, TRANSFORM_LOCAL_TO_TRANS,1);

		// perform world transform
		shadow_obj->ModelToWorld(TRANSFORM_TRANS_ONLY);

		// insert the object into render _list
		_list->Insert(*shadow_obj,0);
	...
	sprintf(work_string,"Lighting [%s]: Ambient=%d, Infinite=%d, Point=%d, Zsort [%s], BckFceRM [%s]", 
