...
void Game::Step()
{
	...
	// draw the sky
	DrawRectangle(0,0, WINDOW_WIDTH, WINDOW_HEIGHT/2, graphics.GetColor(0,35,50), graphics.GetBackSurface());

	// draw the ground
	DrawRectangle(0,WINDOW_HEIGHT/2-1, WINDOW_WIDTH, WINDOW_HEIGHT, graphics.GetColor(20,12,0), graphics.GetBackSurface());
	...
	for (int index_x = 0; index_x < NUM_POINTS_X; index_x++)
		for (int index_z = 0; index_z < NUM_POINTS_Z; index_z++)
		{
			...
			_obj_marker.SetWorldPos(RAND_RANGE(-100,100)-UNIVERSE_RADIUS+index_x*POINT_SIZE,
									_obj_marker.GetMaxRadius() + 50*sin(DEG_TO_RAD(index_x*10+sin(DEG_TO_RAD(index_z))+mcount)),
