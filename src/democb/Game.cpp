...
// 		rgblookup);          // lookup table
	...
	BmpFile* bitmap = new BmpFile("../../assets/chap12/nebblue01.bmp");
...
void Game::Step()
{
 ...
 //	DrawRectangle(0,0, WINDOW_WIDTH, WINDOW_HEIGHT*.5, graphics.GetColor(180,180,220), graphics.GetBackSurface());
	...
	// toggle mipmap mode
	if (Modules::GetInput().KeyboardState()[DIK_M])
	{
		mip_mode = !mip_mode;
	...
	if (Modules::GetInput().KeyboardState()[DIK_1])
	{
		...
		Modules::GetTimer().Wait_Clock(1); // wait, so keyboard doesn't bounce
	} // end if
	...
	for (int index = 0; index < NUM_SCENE_OBJECTS; index++)
	{
		...
		obj_work->SetWorldPos(scene_objects[index].x,
	...
	} else 
	{
		...
		rc.attr =    RENDER_ATTR_INVZBUFFER  
			    | (mip_mode ? RENDER_ATTR_MIPMAP   : 0)
				| (bilinear_mode ? RENDER_ATTR_BILERP : 0)
	...
	graphics.DrawTextGDI(work_string, 0, WINDOW_HEIGHT-14, RGB(0,255,0), graphics.GetBackSurface());
#endif
	...
	if (help_mode)
	{
		...
		graphics.DrawTextGDI("<B>..............Toggle Bilinear Filtering.", 0, text_y+=12, RGB(255,255,255), graphics.GetBackSurface());
		graphics.DrawTextGDI("<M>..............Toggle Mip Mapping.", 0, text_y+=12, RGB(255,255,255), graphics.GetBackSurface());
		graphics.DrawTextGDI("<1>, <2>.........Decrease/Increase Mipdistance.", 0, text_y+=12, RGB(255,255,255), graphics.GetBackSurface());
