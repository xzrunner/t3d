...
// 	Build_Sin_Cos_Tables();
	...
	obj_terrain->GenerateTerrain(TERRAIN_WIDTH,				// width in world coords on x-axis
								...
								"../../assets/chap14/heighlightmap_01.bmp",  // filename of height bitmap encoded in 256 colors
								"../../assets/chap14/sandstone256_256_01.bmp",   // filename of texture map
	...
	texture_copy = new BmpImg(0, 0, 256, 256, 32);
	BmpImg::Copy(texture_copy, 0,0, 
		         obj_terrain->GetTexture(),0,0,
		         256,256);
	...
	for (int index_obj=0; index_obj < NUM_OBJECTS; index_obj++)
	{
		...
		obj_array[index_obj]->SetWorldPos(0, 150, 0);
...
// 	sound.SetVolume(wind_sound_id, 100);
	...
	{
		sprintf(buffer,"../../assets/chap14/fanlightmaps_0%dc.bmp",index+1);
		BmpFile* bitmap = new BmpFile(buffer);
		lightmaps[index] = new BmpImg(0,0,256,256, 32);
		lightmaps[index]->LoadImage32(*bitmap,0,0,BITMAP_EXTRACT_MODE_ABS);
		delete bitmap;                                 
	} // end for index
}
...
//	graphics.FillSurface(graphics.GetBackSurface(), 0);
	...
	unsigned int* sbuffer = (unsigned int*)texture_copy->Buffer();
	unsigned int* lbuffer = (unsigned int*)lightmaps[curr_lightmap]->Buffer();
	unsigned int* dbuffer = (unsigned int*)obj_terrain->GetTexture()->Buffer();
	...
	for (int iy = 0; iy < texture_copy->Height(); iy++)
		for (int ix = 0; ix < texture_copy->Width(); ix++)
		{
			...
			unsigned int spixel = sbuffer[iy*texture_copy->Width() + ix];

			// extract RGB values
			_RGB8888FROM32BIT(spixel, &tmpa,&rs,&gs,&bs); 

			// extract pixel from lightmap bitmap
			unsigned int lpixel = lbuffer[iy*texture_copy->Width() + ix];

			// extract RGB values
			_RGB8888FROM32BIT(lpixel, &tmpa,&rl,&gl,&bl);
			...
			dbuffer[iy*texture_copy->Width() + ix] = _RGB32BIT(255,rf,gf,bf);
...
//	lights[POINT_LIGHT2_INDEX].pos.y = 400;
	...
	// generate rotation matrix around y axis
	mrot = mat4::RotateY(-10.0f);

	// rotate the local coords of the object
	obj_work->Transform(mrot, TRANSFORM_LOCAL_ONLY,1);
