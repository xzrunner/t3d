...
public:
	...
	static const int NUM_LIGHTMAPS          = 9;  // 
...
private:
	...
	RenderObject* obj_work;
	RenderObject* obj_array[NUM_OBJECTS];

	RenderObject* obj_light;
	RenderObject* obj_light_array[NUM_LIGHT_OBJECTS];

	RenderObject* shadow_obj;
	...
	BmpImg* lightmaps[NUM_LIGHTMAPS];   // the light maps
	int curr_lightmap;
	BmpImg* texture_copy;               // copy of the texture
