...
{
	_obj_constant_water = new RenderObject;
	_obj_flat_water = new RenderObject;
	_obj_gouraud_water = new RenderObject;
...
{
	delete _list;
	delete _obj_gouraud_water;
	delete _obj_flat_water;
	delete _obj_constant_water;
}
...
// 	Build_Sin_Cos_Tables();
	...
	vec4 vscale(10.00,10.00,10.00,1),
		vpos(0,0,0,1), 
		vrot(0,0,0,1);

	// load constant shaded water
	_obj_constant_water->LoadCOB("../../assets/chap09/water_constant_01.cob",  
		&vscale, &vpos, &vrot, VERTEX_FLAGS_SWAP_YZ | 
		VERTEX_FLAGS_TRANSFORM_LOCAL  | 
		VERTEX_FLAGS_TRANSFORM_LOCAL_WORLD );

	// load flat shaded water
	_obj_flat_water->LoadCOB("../../assets/chap09/water_flat_01.cob",  
		&vscale, &vpos, &vrot, VERTEX_FLAGS_SWAP_YZ | 
		VERTEX_FLAGS_TRANSFORM_LOCAL | 
		VERTEX_FLAGS_TRANSFORM_LOCAL_WORLD );

	// load gouraud shaded water
	_obj_gouraud_water->LoadCOB("../../assets/chap09/water_gouraud_01.cob",  
		&vscale, &vpos, &vrot, VERTEX_FLAGS_SWAP_YZ | 
		VERTEX_FLAGS_TRANSFORM_LOCAL | 
		VERTEX_FLAGS_TRANSFORM_LOCAL_WORLD );
...
void Game::Step()
{
	...
	_obj_constant_water->Reset();

	// set position of constant shaded water
	_obj_constant_water->SetWorldPos(-50, 0, 120);

	// generate rotation matrix around y axis
	mrot = mat4::RotateX(x_ang) * mat4::RotateY(y_ang) * mat4::RotateZ(z_ang);

	// rotate the local coords of the object
	_obj_constant_water->Transform(mrot, TRANSFORM_LOCAL_TO_TRANS,1);

	// perform world transform
	_obj_constant_water->ModelToWorld(TRANSFORM_TRANS_ONLY);

	// insert the object into render list
	_list->Insert(*_obj_constant_water, false);

	//////////////////////////////////////////////////////////////////////////
	// flat shaded water

	// reset the object (this only matters for backface and object removal)
	_obj_flat_water->Reset();

	// set position of constant shaded water
	_obj_flat_water->SetWorldPos(0, 0, 120);

	// generate rotation matrix around y axis
	mrot = mat4::RotateX(x_ang) * mat4::RotateY(y_ang) * mat4::RotateZ(z_ang);

	// rotate the local coords of the object
	_obj_flat_water->Transform(mrot, TRANSFORM_LOCAL_TO_TRANS,1);

	// perform world transform
	_obj_flat_water->ModelToWorld(TRANSFORM_TRANS_ONLY);

	// insert the object into render list
	_list->Insert(*_obj_flat_water, false);

	//////////////////////////////////////////////////////////////////////////
	// gouraud shaded water

	// reset the object (this only matters for backface and object removal)
	_obj_gouraud_water->Reset();

	// set position of constant shaded water
	_obj_gouraud_water->SetWorldPos(50, 0, 120);

	// generate rotation matrix around y axis
	mrot = mat4::RotateX(x_ang) * mat4::RotateY(y_ang) * mat4::RotateZ(z_ang);

	// rotate the local coords of the object
	_obj_gouraud_water->Transform(mrot, TRANSFORM_LOCAL_TO_TRANS,1);

	// perform world transform
	_obj_gouraud_water->ModelToWorld(TRANSFORM_TRANS_ONLY);

	// insert the object into render list
	_list->Insert(*_obj_gouraud_water, false);
