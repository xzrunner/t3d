...
// 	Build_Sin_Cos_Tables();
	...
	vec4 vscale(5.00,5.00,5.00,1),
		vpos(0,0,0,1), 
		vrot(0,0,0,1);
// 	_obj_player.Load3DSASC("../../assets/chap08/hammer03.asc", &vscale, &vpos, &vrot,
// 		VERTEX_FLAGS_INVERT_WINDING_ORDER | VERTEX_FLAGS_SWAP_YZ);
	_obj_player.LoadCOB("../../assets/chap08/hammer03.cob", &vscale, &vpos, &vrot,
		VERTEX_FLAGS_INVERT_WINDING_ORDER | VERTEX_FLAGS_SWAP_YZ);
...
void Game::Step()
{
	...
	{
		// move forward
		_cam->PosRef().x += 5*sin(DEG_TO_RAD(_cam->Dir().y));
		_cam->PosRef().z += 5*cos(DEG_TO_RAD(_cam->Dir().y));
	} // end if
	...
	if (_obj_player.Dir().y > 360)
		_obj_player.DirRef().y-=360;

	if (_obj_player.Dir().y < 0)
		_obj_player.DirRef().y+=360;

	// ambient rotation
	_obj_player.DirRef().y++;


	// scale object
	if (Modules::GetInput().KeyboardState()[DIK_PGUP])
		_obj_player.Scale(vec3(1.1f,1.1f,1.1f));

	if (Modules::GetInput().KeyboardState()[DIK_PGDN])
		_obj_player.Scale(vec3(.9f,.9f,.9f));
	...
	lights[POINT_LIGHT_INDEX].pos.x = 300*cos(DEG_TO_RAD(plight_ang));
	lights[POINT_LIGHT_INDEX].pos.y = 200;
	lights[POINT_LIGHT_INDEX].pos.z = 300*sin(DEG_TO_RAD(plight_ang));

	if ((plight_ang+=3) > 360)
		plight_ang = 0;

	// move spot light source in ellipse around game world
	lights[SPOT_LIGHT_INDEX].pos.x = 200*cos(DEG_TO_RAD(slight_ang));
	lights[SPOT_LIGHT_INDEX].pos.y = 200;
	lights[SPOT_LIGHT_INDEX].pos.z = 200*sin(DEG_TO_RAD(slight_ang));
	...
	mrot = mat4::RotateY(_obj_player.Dir().y);
	...
	// perform lighting
	if (lighting_mode)
		_obj_player.LightWorld32(*_cam);  

	// insert the object into render list
	_list->Insert(_obj_player, false);

	// insert the object into render list
	//Insert_OBJECT4DV1_RENDERLIST4DV12(&rend_list, &_obj_player,0, 0);
	...
	sprintf(work_string,"pos:[%f, %f, %f] heading:[%f] elev:[%f], polys[%d]", 
		_cam->Pos().x, _cam->Pos().y, _cam->Pos().z, _cam->Dir().y, _cam->Dir().x, debug_polys_rendered_per_frame); 
	...
	if (help_mode)
	{
		...
		graphics.DrawTextGDI("<RIGHT ARROW>....Rotate object right.", 0, text_y+=12, RGB(255,255,255), graphics.GetBackSurface());
		graphics.DrawTextGDI("<LEFT ARROW>.....Rotate object left.", 0, text_y+=12, RGB(255,255,255), graphics.GetBackSurface());
		graphics.DrawTextGDI("<UP ARROW>.......Move camera forward.", 0, text_y+=12, RGB(255,255,255), graphics.GetBackSurface());
		graphics.DrawTextGDI("<DOWN ARROW>.....Move camera backward.", 0, text_y+=12, RGB(255,255,255), graphics.GetBackSurface());
		graphics.DrawTextGDI("<PG UP>..........Scale object down", 0, text_y+=12, RGB(255,255,255), graphics.GetBackSurface());
		graphics.DrawTextGDI("<PG DWN>.........Scale object down", 0, text_y+=12, RGB(255,255,255), graphics.GetBackSurface());
