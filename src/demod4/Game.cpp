	...
	, main_instance(instance)
{
	...
	bspeditor::Context::Instance()->obj_moveable = new RenderObject;
	zbuffer = new ZBuffer;
	background_bmp = NULL;
}
...
// 	beep0_sound_id = DSound_Load_WAV("../menuitem.wav");
	...
	// load the object in
	bspeditor::Context::Instance()->obj_moveable->LoadCOB("../../assets/chap13/rec_flat_textured_01.cob",
...
void Game::Step()
{
    ...
    // reset the object (this only matters for backface and object removal)
	RenderObject* obj_moveable = bspeditor::Context::Instance()->obj_moveable;
	...
	obj_moveable->SetWorldPos(500*cos(DEG_TO_RAD(rec_count)),
		                      40 + 20*sin(DEG_TO_RAD(rec_count*2)),
							  500*sin(DEG_TO_RAD(rec_count)));

    if ((rec_count+=.5) > 360)
       rec_count =- 360;

    // attempt to cull object   
    if (!obj_moveable->Cull(*context->cam, CULL_OBJECT_XYZ_PLANES))
       {
       // create an identity to work with
       mrot = mat4::Identity();
/*
...
*/
	   ...
	   mrot = mat4::RotateY(-rec_count);

       // rotate the local coords of the object
       obj_moveable->Transform(mrot, TRANSFORM_LOCAL_TO_TRANS,1);

       // perform world transform
       obj_moveable->ModelToWorld(TRANSFORM_TRANS_ONLY);

       // insert the object into render list
       list->Insert(*obj_moveable,0);
       } // end if
